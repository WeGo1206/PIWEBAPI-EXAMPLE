...
def main():
    ...
    piwebapi_url = str(input('Enter the PI Web API url: ') or 'https://srv009-PIVISION.pi.dev.org/piwebapi')
    af_server_name = str(input('Enter the Asset Server Name: ') or 'SRV005-AF1')
    pi_server_name = str(input('Enter the PI Server Name: ') or 'SRV003-DA1')
    piwebapi_user = str(input('Enter the user name: ') or 'PIDEV\\PIAdm')
    piwebapi_password = str(getpass.getpass('Enter the password: ') or 'VwG0lf43*')
    piwebapi_security_method = str(
        input('Enter the security method, Basic or Kerberos: ') or 'Kerberos')
    piwebapi_security_method = piwebapi_security_method.lower()
    verify_ssl_string = str(input('Verify certificates? (Y/N): ') or 'Y')
    
    if (verify_ssl_string.upper() == "N"):
        print('Not verifying certificates poses a security risk and is not recommended')
        verify_ssl = False
    else:
        verify_ssl = "D:\\SSLCertificates\\srv009-pivision-full.pem"
